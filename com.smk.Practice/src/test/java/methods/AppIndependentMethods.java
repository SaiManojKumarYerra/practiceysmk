...
public class AppIndependentMethods extends DriverScript {
	 ...
	 *
	 ************************************************/
	public boolean clickObjectInTable(WebDriver oDriver, By objBy, String strExpectedValue, ExtentTest test)
	{
		List<WebElement> objEle = null;
		...
		try {
			objEle = oDriver.findElements(objBy);
			if(objEle.size() > 0) {
				reports.writeResult(oDriver, "Pass", "The element '"+String.valueOf(objBy)+"' was displayed successful", test);
				for(int i=0; i<objEle.size();i++) {
					if(objEle.get(i).getText().contains(strExpectedValue)) {
						objEle.get(i).click();
						break;
					}
				}
			...
			if(objEle.size() > 0) {
				switch(strObjectType.toLowerCase()) {
					...
					case "dropdown":
						...
						break;
					default:
						reports.writeResult(oDriver, "Fail", "Invalid name for the objectType '"+strObjectType+"'", null);
						return false;
				}
